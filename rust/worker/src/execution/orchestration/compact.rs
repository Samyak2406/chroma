...
// TODO(tanujnay112): Add a test to fail one apply but succeed others.
#[allow(clippy::too_many_arguments)]
...
mod tests {
    ...
    use chroma_system::{Dispatcher, DispatcherConfig, Orchestrator, System};
    use chroma_types::{
    ...
    async fn test_broken_apply() {
        ...
        let collection: Collection = sysdb
            .get_collection_with_segments(collection_uuid)
            .await
            .unwrap()
            .collection;
    ...
    async fn test_compaction_with_empty_logs_from_inserts_and_deletes() {
        ...
        println!(
            ...
            collection_after_compaction
                .collection
                .size_bytes_post_compaction
        );
        println!(
            "Collection log position: {}",
            collection_after_compaction.collection.log_position
        );

        assert_eq!(
            collection_after_compaction.collection.total_records_post_compaction, 0,
            "Collection should have 0 records post-compaction when all inserts/deletes net out to empty, but got {} records",
            collection_after_compaction.collection.total_records_post_compaction
        );

        assert_eq!(
            collection_after_compaction.collection.size_bytes_post_compaction, 0,
            "Collection should have 0 bytes post-compaction when all inserts/deletes net out to empty, but got {} bytes",
            collection_after_compaction.collection.size_bytes_post_compaction
        );

        assert_eq!(
            collection_after_compaction.collection.log_position, 251,
